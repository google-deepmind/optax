...
with both quadratic and Rosenbrock functions, warm-up runs, and convergence analysis.
"""
...
def run_single_comparison(name, initial_params, objective_fn_jax, objective_fn_torch, bounds=None, verbose=False):
    ...
    for _ in range(3):
        optimize_optax_lbfgs(initial_params, objective_fn_jax, max_iter=5)
        if bounds:
            optimize_optax_lbfgs_b(initial_params, objective_fn_jax, bounds=bounds, max_iter=5)
...
def run_complete_comparison():
    ...
    print(f"First-run times would be significantly higher due to compilation overhead.")
