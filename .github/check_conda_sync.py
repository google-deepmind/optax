...
def main() -> None:
  ...
  meta = yaml.safe_load(jinja2.Template(raw).render())
  conda_pin = None
  ...
  if op_pypi != op_conda or packaging.version.Version(
      ver_pypi
  ) != packaging.version.Version(ver_conda):
    warn(
        f"jax pin mismatch - PyPI: jax{op_pypi}{ver_pypi}, "
        f"conda: jax{op_conda}{ver_conda}"
    )
    print(
        "Please open a PR at "
        "https://github.com/conda-forge/optax-feedstock "
        "to update recipe/meta.yaml: jax "
