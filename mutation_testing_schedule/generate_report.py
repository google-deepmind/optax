...
def load_results():
    ...
    results_file = Path("mutation_testing/results/mutation_results.json")
    with open(results_file, 'r') as f:
        return json.load(f)

def load_source():
    """Load the source file."""
    source_file = Path("optax/schedules/_schedule.py")
...
def generate_markdown_report(results, source_code):
    ...
    report = []
    report.append("# Mutation Testing Report: Optax Schedules")
    ...
    report.append(f"- **Test Suite Execution Time:** ~13 seconds for _schedule_test.py (72 tests)")
    ...
    report.append("Mutation testing results broken down by individual schedule functions:\n")
    ...
    report.append(f"The mutation testing study revealed that the Optax schedules module has a **{results['mutation_score']:.2f}% mutation score**, ")
    ...
    report.append("- `optax/schedules/_schedule.py` - Source code under test")
    report.append("- `mutation_testing/results/mutations_diff.txt` - Unified diff file with all mutations")
    report.append("- `mutation_testing/results/mutation_results.json` - Detailed JSON results")
    report.append("- `mutation_testing/single_file_mutation.py` - Mutation testing script")
...
def main():
    ...
    output_file = Path("mutation_testing/MUTATION_TESTING_REPORT.md")
