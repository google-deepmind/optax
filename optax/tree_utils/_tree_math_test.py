...
class TreeUtilsTest(absltest.TestCase):
  ...
  def test_reduce_accross_trees(self):
    mean_of_two_fun = lambda x: sum(x) / 2
    trees = [self.tree_a_dict, self.tree_a_dict]
    expected = self.tree_a_dict
    got = tu.reduce_across_trees(mean_of_two_fun, trees)
    chex.assert_trees_all_close(expected, got)

  def test_random_like_tree(self):
    rand_tree_a = tu.random_like_tree(self.rng_jax, self.tree_a)
    rand_tree_b = tu.random_like_tree(self.rng_jax, self.tree_b)
    rand_tree_a_dict = tu.random_like_tree(self.rng_jax, self.tree_a_dict_jax)
    rand_tree_b_dict = tu.random_like_tree(self.rng_jax, self.tree_b_dict_jax)
    rand_array_a = tu.random_like_tree(self.rng_jax, self.array_a)
    rand_array_b = tu.random_like_tree(self.rng_jax, self.array_b)
    eps = 1e-6
