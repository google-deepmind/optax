...
def scale_by_dog(
  ...
  def init_fn(params: base.Params) -> DoGState:
    ...
    if init_step_type == "distance":
      ...
      r_epsilon = init_step_value * (1 + otu.tree_l2_norm(params))
  ...
  def update_fn(
    ...
    dist = otu.tree_l2_norm(otu.tree_sub(state.init_params, params))
    ...
    if init_step_type == "learning_rate":
      learning_rate = jnp.where(state.is_init_step, init_step_value, learning_rate)
...
def dog(
    ...
    init_step: tuple[Literal["distance", "learning_rate", "heuristic"], float] = ("heuristic", 1e-6),
    eps: float = 1e-8,
  ...
  Args:
    ...
    init_step: Initial step specification. Consists of a pair ``(tag, value)``, where
      ``value`` is a float and ``tag`` is a string, which must be one of ``distance``,
      ``learning_rate``, or ``heuristic``.
      ``distance`` sets the initial distance :math:`r_0` (:math:`r_\epsilon` in the
      paper) to the given value.
      ``learning_rate`` sets the initial learning rate :math:`\eta_0` to the given value.
      ``heuristic`` sets  :math:`r_0 = \alpha (1 + \|x_0\|)`, where :math:`\alpha` is
      the given value. The suggested value of :math:`\alpha` is 1e-6, unless the model
      uses batch normalization, in which case the suggested value is 1e-4.
      As discussed in the paper, the value should be small enough to ensure that the
      initial update step will be small enough to not cause the model to diverge.
