...
def scale_by_mars(
  ...
  def update_fn(updates, state, params=None):
    ...
    raw_correction = jax.tree_util.tree_map(
        lambda g, pg: correction_coeff * (g - pg),
    ...
    correction = jax.tree_util.tree_map(
        lambda c: jnp.where(is_step_zero, jnp.zeros_like(c), c),
        correction
    )

    new_updates = jax.tree_util.tree_map(
        lambda adam, corr: adam + corr,
        adam_direction, correction
    )
...
def mars_adamw(
  ...
  Args:
    ...
    mask: A tree with same structure as (or prefix of) params containing booleans,
      which indicate whether to apply the weight decay to the corresponding
      parameter.
