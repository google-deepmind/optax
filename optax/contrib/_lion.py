"""
Lion Optimzer (EvoLved Sign Momentum)
...
def scale_by_lion(
    ...
    Args:
        ...
        mu_dtype: Optional dtype to be used for the momentum accumulator; if None 
    ...
    def update_fn(updates, state:ScaleByLionState, params=None):
        ...
        m_tilde = jax.tree.map(lambda m, g: b1 * m + (1.0 - b1) * g, state.mu, updates)
        
        # the returned update is sign(m_tilde)
        signed_update = jax.tree.map(jnp.sign, m_tilde)
        
        # Compute the stored momentum (m_t)
        new_mu = jax.tree.map(lambda m, g: b2 * m + (1.0 - b2) * g, state.mu, updates)
...
def lion(
    ...
    Args:
        ...
        weight_decay : Weight decay, Strength of regularization. Note that this 
                       Multiplied with learning_rate               
        mask : A tree with the same structure as the params PyTree,
               ...
               apply the weight decay to, and False to those you want to skip.
