...
class MuonTest(parameterized.TestCase):
  ...
  def test_reshape_and_update_single_param(self, preconditioning):
    ...
    params = {'w': w}
    updates, _ = get_updates(params, preconditioning=preconditioning)
  ...
  def test_dim_nums_combinations(self, preconditioning):
    ...
    _, state = get_updates(params, preconditioning=preconditioning)
  ...
  def test_newton_schulz(self):
    ...
    with self.subTest('Real Newton--Schulz produces an orthonormal matrix'):
      msg = ('Real Newton–Schulz did not orthogonalize correctly.\nGram:'
             f' \n{gram_real}')
      np.testing.assert_allclose(gram_real, jnp.eye(mat_real.shape[1]),
    ...
    with self.subTest('Complex Newton--Schulz produces a unitary matrix'):
      msg = ('Complex Newton–Schulz did not produce a unitary matrix.'
             f'\nGram:\n{gram_complex}')
      np.testing.assert_allclose(gram_complex, jnp.eye(mat_complex.shape[1]),
                                 atol=atol, rtol=rtol, err_msg=msg)
  ...
  def test_complex_mixed_target(self, target, dtype, adaptive, nesterov):
    ...
    opt = _muon.muon(learning_rate=1e-2, adaptive=adaptive, nesterov=nesterov)
  ...
  def test_aol_numerical_difference(self):
    ...
    def _get_updates(preconditioning, **kwargs):
      opt = _muon.muon(learning_rate=0.1,
                       preconditioning=preconditioning, **kwargs)
      ...
      for leaf in jax.tree_util.tree_leaves(updates):
        self.assertFalse(jnp.isnan(leaf).any(),
                         f'Found NaN values in {preconditioning}')
      return updates

    u_std = _get_updates('frobenius')
    u_aol = _get_updates('aol', ns_coeffs='dion')
    u_schatten = _get_updates('schatten', ns_coeffs='dion')

    with self.assertRaises(AssertionError,
                           msg='AOL should differ from Frobenius and Schatten'):
      test_utils.assert_trees_all_close(u_std, u_aol)
...
class AdaMuonTest(parameterized.TestCase):
  ...
  def test_adamuon_produces_valid_updates(self):
    ...
    state = opt.init(params)
    updates, _ = opt.update(params, state, params=params)
    self.assertEqual(updates['w'].shape, (10, 10))
