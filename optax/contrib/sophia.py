...
def scale_by_sophia(
  ...
  """
  if not 0 <= b1 < 1:
    raise ValueError(f'{b1=} must be in the range [0, 1).')
  if not 0 <= b2 < 1:
    raise ValueError(f'{b2=} must be in the range [0, 1).')
  if rho <= 0:
    raise ValueError(f'{rho=} must be > 0.')
  if batch_size <= 0:
    raise ValueError(f'{batch_size=} must be > 0.')
  if update_hessian_every < 1:
    raise ValueError(f'{update_hessian_every=} must be >= 1.')
