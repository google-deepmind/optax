...
_MAIN_OPTIMIZERS_UNDER_TEST = [
    ...
    {"opt_name": "l_dog", "opt_kwargs": {"learning_rate": 1.0}},
    ...
    {"opt_name": "sophia", "opt_kwargs": {"learning_rate": 1e-2}},
    {"opt_name": "galore", "opt_kwargs": {"learning_rate": 1e-2, "rank": 8}},
]
...
class ContribTest(parameterized.TestCase):
    ...
    def test_optimizers_accept_extra_args(
        ...
        def step(params, state):
            ...
            update_kwargs = {
                "unexpected_extra_args_your_optimizer_doesnt_expect": 1
            }
        ...
        with self.subTest(
            "Test that the optimizer doesn't recompile on 2nd call"
        ):
            params = initial_params
            state = opt.init(params)
            params, state = step(params, state)
    ...
    def test_optimizers_can_be_wrapped_in_inject_hyperparams(
        ...
        if wrapper_name == "reduce_on_plateau":
            ...
            self.skipTest(
                "reduce_on_plateau is not supported by inject_hyperparams."
            )
        ...
        for uninjectable_hparam in [
            ...
            if (
                uninjectable_hparam
                in inspect.signature(factory).parameters.keys()
            ):
        ...
        if opt_name == "sophia":
            ...
            inject_update_fn = functools.partial(
                opt_inject.update, obj_fn=obj_fn
            )
        else:
        ...
        with self.subTest("Equality of updates."):
            test_utils.assert_trees_all_close(
                updates_inject, updates, rtol=1e-5
            )
    ...
    def test_state_shape_dtype_shard_stability(
        ...
        with utils.x64_precision(dtype in (jnp.float64, jnp.complex128)):
            ...
            def step(params, state):
                value, updates = jax.value_and_grad(objective)(params)
                value = value.astype(
                    jnp.float16 if dtype != jnp.float16 else jnp.bfloat16
                )  # confuse dtype intentionally
                ...
                updates, state = jax.lax.cond(
                    cond,
                    lambda: opt.update(updates, state, params, **update_kwargs),
                    lambda: (params, state),
                )
                params = update.apply_updates(params, updates)
                return params, state
    ...
    def test_optimizers_accept_learning_rate_schedule_if_type_annotated_as_such(
        ...
        if lr_arg is None or lr_arg.annotation not in (
        ...
        with self.subTest("Test that optimization works"):
            ...
            test_utils.assert_trees_all_close(
                params_with_schedule, params_with_lr
            )
