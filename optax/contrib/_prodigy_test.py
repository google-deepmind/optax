...
def _abs_sum(tree):
  return sum(jnp.abs(x).sum() for x in jax.tree.leaves(tree))


def _updates(params, **kwargs):
  opt = _prodigy.prodigy(learning_rate=1e-1, **kwargs)
...
class ProdigyTest(parameterized.TestCase):
  ...
  def test_zero_grads_take_no_step(self):
    """A zero denominator must not produce a non-finite update."""
    params = _params()
    opt = _prodigy.prodigy(learning_rate=1e-1)
    ...
    # The estimate of `D` and the step count must be frozen, not merely finite.
  ...
  def test_zero_learning_rate_takes_no_step(self):
    ...
    params = _params()
    opt = _prodigy.prodigy(learning_rate=0.0)
    grads = jax.tree.map(lambda p: jnp.full_like(p, 0.1), params)
    state = opt.init(params)
  ...
  def test_schedule_starting_at_zero_does_not_wedge(self):
    """A warmup starting at zero must not freeze the schedule forever."""
    params = _params()
    warmup = lambda c: jnp.where(c < 2, 0.0, 1e-1)
    opt = _prodigy.prodigy(learning_rate=warmup)
    grads = jax.tree.map(lambda p: jnp.full_like(p, 0.1), params)
    state = opt.init(params)
    for _ in range(2):
      updates, state = opt.update(grads, state, params)
    ...
    self.assertGreater(_abs_sum(updates), 0.0)

  def test_safeguards_do_not_change_the_common_case(self):
    ...
    self.assertTrue(jnp.isfinite(_abs_sum(params)))
