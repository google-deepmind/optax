...
class ClippingTest(absltest.TestCase):
    ...
    def test_adaptive_grad_clip_with_axis(self):
        ...
        clipped_conv3d, _ = agc_conv3d.update(
            [conv3d_grad], None, [conv3d_param]
        )
        clipped_conv3d = clipped_conv3d[0]

        # Test linear layer clipping
        clipped_linear, _ = agc_linear.update(
            [linear_grad], None, [linear_param]
        )
        ...
        conv3d_grad_norm = _clipping.unitwise_norm(
            clipped_conv3d, axis=(0, 1, 2)
        )
        conv3d_param_norm = _clipping.unitwise_norm(
            conv3d_param, axis=(0, 1, 2)
        )
    ...
    def test_unitwise_norm_with_axis(self):
        ...
        norm_channels = _clipping.unitwise_norm(
            x, axis=(3, 4)
        )  # Over channel dims
    ...
    def test_adaptive_grad_clip(self):
        ...
        for i in range(1, STEPS + 1):
            ...
            # Check that the clipper actually works and upd_norm is < clip_r * param_norm.
            ...
            # Check that continuously clipping won't cause numerical issues.
            updates_step, _ = clipper.update(
                self.per_step_updates, None, params
            )
            chex.assert_trees_all_close(updates, updates_step)
    ...
    def test_per_example_layer_norm_clip(self):
        ...
        with self.subTest(name="Uniform Variant"):
            sum_clipped_grads, num_clipped = (
                _clipping.per_example_layer_norm_clip(
