...
def freeze(mask: Union[bool, chex.ArrayTree]) -> "base.GradientTransformation":
    """
    Create a transformation that zeros out gradient updates according to `mask`,
    ...
    .. seealso::
        :func:`optax.transforms.selective_transform` : For partitioning updates so only un-frozen parameters are optimized.
    """
...
def selective_transform(
    ...
    Args:
      ...
      freeze_mask: A *static* mask (i.e., not dependent on runtime values or
        ...
        • a scalar bool (or 0-d JAX bool array) to freeze everything (True)
          or nothing (False), or
        • a PyTree of booleans mirroring the parameter tree, marking each
          leaf to freeze (True) or train (False).
    ...
    .. seealso::
        :func:`optax.transforms.freeze` : For simply zeroing out gradients according to a mask.
    """
