...
from optax._src import alias, base, update
from optax.transforms import _freezing

# Data setup for Freeze shortcuts test
PARAMS_FLAT = {"a": jnp.zeros(1), "b": jnp.ones(2)}
PARAMS_NESTED = {
    "layer1": [jnp.zeros((2, 3)), {"bias": jnp.ones(3)}],
    "layer2": {"w": jnp.full((4, 2), 2.0), "b": jnp.full(4, -1.0)},
    "misc": (jnp.array(5.0),),
}
GRAD_FLAT = {"a": jnp.array([1.0]), "b": jnp.array([2.0, 3.0])}
GRAD_NESTED = {
    "layer1": [jnp.full((2, 3), 1.0), {"bias": jnp.full(3, 2.0)}],
    "layer2": {"w": jnp.full((4, 2), 3.0), "b": jnp.full(4, 4.0)},
    "misc": (jnp.array(1.0),),
}
    ...
    @parameterized.named_parameters(
        [
            ...
            (
                ...
                GRAD_FLAT,
                {"a": False, "b": True},
                {"a": GRAD_FLAT["a"], "b": jnp.array([0.0, 0.0])},
            ),
            ...
            (
                ...
                GRAD_FLAT,
                True,
                {"a": jnp.array([0.0]), "b": jnp.array([0.0, 0.0])},
            ),
            # flat: freeze nothing
            ...
            (
                ...
                {
                    "layer1": [
                        jnp.zeros_like(GRAD_NESTED["layer1"][0]),
            ...
            (
                ...
                },
                {
                    ...
                    "layer2": {
                        "w": jnp.zeros_like(GRAD_NESTED["layer2"]["w"]),
...
class SelectiveTransformTest(parameterized.TestCase):
            ...
            (
                ...
                },
                {
                    "layer1": [
                        ...
                        {
                            "bias": PARAMS_NESTED["layer1"][1]["bias"]
                            - GRAD_NESTED["layer1"][1]["bias"]
                        },
                    ],
                    "layer2": {
                        "w": PARAMS_NESTED["layer2"]["w"] - GRAD_NESTED["layer2"]["w"],
                        "b": PARAMS_NESTED["layer2"]["b"] - GRAD_NESTED["layer2"]["b"],
                    },
    ...
    def test_selective_transform_effect(
        self, params, grads, freeze_mask, expected_params
    ):
        """Tests that selective_transform only updates the unfrozen leaves."""
        inner_opt = alias.sgd(learning_rate=1.0)
        optimizer = _freezing.selective_transform(inner_opt, freeze_mask)
    ...
    def test_nested_train_all(self):
        ...
        opt = _freezing.selective_transform(alias.sgd(1.0), mask)
    ...
    @parameterized.named_parameters(
        [
            ("py_bool", True),
            ("jax_bool", jnp.array(True)),
        ]
    )
    def test_scalar_freeze_all(self, scalar_mask):
        opt = _freezing.selective_transform(alias.sgd(1.0), scalar_mask)
    ...
    def test_selective_bad_structure(self):
        ...
        opt = _freezing.selective_transform(alias.sgd(1.0), bad_mask)
        with self.assertRaisesRegex(ValueError, "Dict key mismatch"):
    ...
    def test_partial_prefix_mask_behavior(self):
        ...
        optimizer = _freezing.selective_transform(inner_opt, mask)
        state = optimizer.init(params)
        updates, _ = optimizer.update(grads, state, params)
        new_params = update.apply_updates(params, updates)
