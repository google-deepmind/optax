...
class MicrobatchingTest(parameterized.TestCase):
  ...
  def test_micro_value_and_grad_basic(self, keep_batch_dim):
    ...
    grad_fn = microbatching.micro_value_and_grad(
        mean_squared_loss,
        argnums=0,
        batch_argnums=(1, 2),
    ...
    expected_grad = -(0 + 1 + 2 + 3) / 4
    expected_squared_grad = (0**2 + 1**2 + 2**2 + 3**2) / 4
