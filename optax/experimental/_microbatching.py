...
AuxiliaryOutput = collections.namedtuple('Aux', ['values', 'metrics', 'aux'])
ValueAndGradFn: TypeAlias = Callable[..., tuple[Any, AuxiliaryOutput]]
...
def micro_value_and_grad(
  ...
  """Create a function to compute, transform, and average per-example gradients.

  This function is similar to jax.value_and_grad, but works at the level of
  size-1 batches.  It can be useful to e.g.,
  ...
  * computing scalar or low-dimensional gradient metrics on a per-example basis.
  ...
  Example Usage (see https://arxiv.org/abs/2510.00236):
    ...
    >>> (grads, squared_grads), aux = micro_value_and_grad(
    ...     mean_squared_loss,
    ...     argnums=0,
    ...     batch_argnums=(1,2),
  ...
  Args:
    ...
    argnums: The indices of arguments to differentiate with respect to.
    batch_argnums: The indices of arguments with a batch axis.
  ...
  micro_fun = gvmap(
      grad_fn,
      in_axes=in_axes,
      accumulator=(AccumulationType.MEAN, AccumulationType.CONCAT),
      microbatch_size=microbatch_size
  )
  if keep_batch_dim:
