...
class AggregatorsTest(parameterized.TestCase):
  ...
  def test_aggregation_and_accumulation_match_standard(self):
    ...
    opt = aggregate.process(
        ...
        aggregate.average_incrementally_updates(
            per_elt_axis=0, num_microbatches=1
        ),
        base_opt,
    )
    ...
    with self.subTest('aggregation matches standard'):
      chex.assert_trees_all_close(std_params, agg_params)
      chex.assert_trees_all_close(
          std_metrics['full_batch_loss'], agg_metrics['full_batch_loss']
      )

    opt = aggregate.process(
        base.identity(),
        aggregate.average_incrementally_updates(
            per_elt_axis=None, num_microbatches=2
        ),
        base_opt,
    )
    acc_params, acc_metrics = _train(opt, num_microbatches=2)
    ...
    opt = aggregate.process(
        base.identity(),
        aggregate.average_incrementally_updates(
            per_elt_axis=0, num_microbatches=2
        ),
        base_opt,
    )
    agg_acc_params, agg_acc_metrics = _train(opt, num_microbatches=2)
  ...
  def test_mean_variance_ema_match_standard(self, ema_decay: float = 0.99):
    ...
    opt = aggregate.add_mean_variance_to_opt(base_opt, ema_decay)
    mean_var_agg_params, mean_var_agg_metrics = _train(opt)

    with self.subTest(
        'mean variance ema with aggregation training matches standard'
    ):
      chex.assert_trees_all_close(std_params, mean_var_agg_params, rtol=1e-2)
      chex.assert_trees_all_close(
          std_metrics['full_batch_loss'],
          mean_var_agg_metrics['full_batch_loss'],
          rtol=1e-2,
      )
    with self.subTest('var grads ema matches var grads'):
      if ema_decay == 0.0:
        chex.assert_trees_all_close(
            mean_var_agg_metrics['var_grads_ema'],
            mean_var_agg_metrics['var_grads'],
        )

    opt = aggregate.add_mean_variance_to_opt(
        base_opt, ema_decay, num_microbatches=2
    )
    mean_var_acc_params, mean_var_acc_metrics = _train(opt, num_microbatches=2)
    with self.subTest(
        'mean variance ema with accumulation training matches standard'
    ):
      chex.assert_trees_all_close(std_params, mean_var_acc_params, rtol=1e-2)
      chex.assert_trees_all_close(
          std_metrics['full_batch_loss'],
    ...
    with self.subTest(
        'var grads ema with accumulation matches var grads ema with'
        ' aggregation'
    ):
      chex.assert_trees_all_close(
          mean_var_agg_metrics['var_grads_ema'],
