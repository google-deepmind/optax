  ...
  def update_fn(updates, state, params=None):
    ...
    def _clip_fn(u):
      clip_denom = jnp.maximum(
        1.0, jnp.sqrt(jnp.mean(numerics.abs2(u))) / threshold)
      return u / clip_denom
...
def unitwise_norm(x: chex.Array) -> chex.Array:
  ...
  if jnp.squeeze(x).ndim <= 1:  # Scalars and vectors
    squared_norm = jnp.sum(numerics.abs2(x), keepdims=True)
  ...
  # to modify this line.
    ...
    squared_norm = jnp.sum(numerics.abs2(x), axis=0, keepdims=True)
  elif x.ndim == 4:  # Conv kernels of shape HWIO
    squared_norm = jnp.sum(numerics.abs2(x), axis=(0, 1, 2), keepdims=True)
  else:
    raise ValueError(
