...
from optax._src import alias, combine, transform, update
...
class TransformTest(parameterized.TestCase):
    ...
    def test_add_decayed_weights(self, weight_decay_value_or_schedule, mask):
        ...
        is_lambda_schedule = (
            callable(weight_decay_value_or_schedule)
            and weight_decay_value_or_schedule.__name__ == "<lambda>"
        )
        ...
        for step in range(3):
            if callable(current_weight_decay_source):
                ...
                is_masked_schedule_state = isinstance(state, tuple) and any(
                    isinstance(s, transform.ScaleByScheduleState) for s in state
                )
                if is_direct_schedule_state or is_masked_schedule_state:  # masked state
                    ...
                    is_masked_tuple = (
                        ...
                        and len(state) == 2
                        and isinstance(state[0], transform.ScaleByScheduleState)
                    )
