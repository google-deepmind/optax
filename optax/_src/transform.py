...
def _reject_complex(params):
    if any(jnp.iscomplexobj(x) for x in jax.tree.leaves(params)):
        raise ValueError(
            "This transformation does not support complex parameters."
        )
...
def scale_by_rss(
    ...
    def init_fn(params):
        return ScaleByRssState(
            sum_of_squares=optax.tree.full_like(
                params, initial_accumulator_value
            )
        )
    ...
    *,
    ...
    def update_fn(updates, state, params=None):
        ...
        updates = jax.tree.map(
            lambda m, v: (
                None if m is None else m / (jnp.sqrt(v + eps_root) + eps)
            ),
            mu_hat,
            nu_hat,
    ...
    def update_fn(updates, state, params=None):
        ...
        updates = jax.tree.map(
            lambda g, cur_e_g, prev_e_x: (
                (jnp.sqrt(prev_e_x + eps) / jnp.sqrt(cur_e_g + eps)) * g
            ),
            updates,
            e_g,
            state.e_x,
...
def scale_by_adan(
    ...
    def update_fn(updates, state, params=None):
        ...
        denom = jax.tree.map(
            lambda n_hat: jnp.sqrt(n_hat + eps_root) + eps, n_hat
        )
...
def scale_by_belief(
    ...
    *,
    ...
    def update_fn(updates, state, params=None):
        ...
        nu = optax.tree.update_moment_per_elem_norm(
            prediction_error, state.nu, b2, 2
        )
...
def scale_by_rprop(
    ...
    def update_fn(updates, state, params=None):
        ...
        updates = jax.tree.map(
            lambda s, g, prev_g: jnp.where(
                s < 0, jnp.zeros_like(prev_g), prev_g
            ),
            sign,
            prev_updates,
            state.prev_updates,
...
def scale_by_sm3(
    ...
    def _new_accum(g, v):
        ...
        if g.ndim < 2:
            ...
            return coeffs[0] * g**2 + coeffs[1] * functools.reduce(
                jnp.minimum, v
            )
...
def scale_by_novograd(
    ...
    def init_nu(grads, nu):
        return jax.tree.map(
            lambda g, n: nu_addition(g).astype(n.dtype), grads, nu
        )
...
def scale_by_polyak(
    ...
    def update_fn(
        ...
        **extra_args,
        ...
        if variant == "sps":
            ...
            raise ValueError(
                f"Invalid argument value for Polyak SGD: {variant=}"
            )
    ...
    return base.GradientTransformationExtraArgs(
        base.init_empty_state, update_fn
    )
    ...
    def init_fn(params: base.Params) -> ScaleByLBFGSState:
        ...
        stacked_zero_params = jax.tree.map(
            lambda leaf: jnp.zeros(
                (memory_size,) + leaf.shape, dtype=leaf.dtype
            ),
            params,
        )
